...
# Multi-Account MT5 Dispatcher (Smart Mode: LOGIN_SWITCH & MULTI_TERMINAL)
# Solusi tuntas 100% bebas dari MCP bind error socket 22346 & IPC timeout
# =====================================================
...
def get_multi_account_funds_info(strategy_name: str) -> list[dict]:
    """
    Mengambil rincian dana & kesehatan semua akun target dengan metode Login Switch (Cepat & Stabil).
    """
    ...
    for acc_info in accounts:
        # Jika akun yang diminta sama dengan yang sedang login di terminal, langsung baca
        current_acc = mt5.account_info()
        is_already_active = current_acc and current_acc.login == acc_info['login']
        ...
        if not is_already_active:
            ...
            if not logged_in:
                err = mt5.last_error()
                print(cprint(f"⚠️ Gagal login audit akun {acc_info['name']} ({acc_info['login']}): {err}", Colors.YELLOW))
                results.append({
                    "key": acc_info['key'],
                    "name": acc_info['name'],
                    "login": acc_info['login'],
    ...
    # Kembalikan terminal ke akun awal jika berbeda
    if initial_login > 0:
        first_acc = accounts[0]
        if initial_login != accounts[-1]['login']:
            mt5.login(login=first_acc['login'], password=first_acc['password'], server=first_acc['server'], timeout=15000)

    return results

def _execute_single_account_order(acc_info: dict, strategy_name: str, payload: dict) -> dict:
    """
    Mengeksekusi order pada 1 akun tertentu via login switch.
    """
    # 1. Switch login ke akun target
    logged_in = mt5.login(
        login=acc_info['login'],
        password=acc_info['password'],
        server=acc_info['server'],
    ...
    if not logged_in:
        err = mt5.last_error()
        err_msg = f"Gagal login akun {acc_info['name']} ({acc_info['login']}): {err}"
...
def dispatch_multi_account_order(strategy_name: str, payload: dict) -> tuple[MultiOrderResult | None, list[dict]]:
    """
    Fungsi penembak utama: Mengeksekusi order ke seluruh akun target secara cepat via Login Switching.
    ...
    for acc in accounts:
        try:
            res = _execute_single_account_order(acc, strategy_name, payload)
            results.append(res)
        except Exception as exc:
            print(cprint(f"⚠️ Error eksekusi akun {acc['key']}: {exc}", Colors.RED))
            results.append({"key": acc['key'], "success": False, "error": str(exc), "retcode": -1, "order": None})

    # Kembalikan terminal ke login akun pertama (ACC1)
    mt5.login(login=first_acc['login'], password=first_acc['password'], server=first_acc['server'], timeout=15000)

    # Temukan akun primer pertama yang berhasil (ACC1)
    primary_res = None
    for r in results:

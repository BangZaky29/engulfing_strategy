...
def run_mrcv_bot():
    ...
    symbol = os.getenv("MRCV_SYMBOL", "BTC")
    tf_str = os.getenv("MRCV_TIMEFRAME", "M5")
    
    # Pastikan symbol aktif
    if not mt5.symbol_select(symbol, True):
        print(cprint(f"❌ Gagal memilih symbol {symbol}", Colors.RED))
        return
        
    mrcv_magic = int(os.getenv("MRCV_MAGIC_NUMBER", "999000"))
    mrcv_magics = [mrcv_magic]
    ...
    mrcv_group_jid = os.getenv("MRCV_GROUP_JID", "120363430592783067@g.us")
    send_mrcv_wa_notif(start_msg, "MRCV_START", target_jid=mrcv_group_jid, include_header=False)
    ...
    while True:
        try:
            ...
            if is_rcs_hedge and not last_hedge_status:
                ...
                mrcv_group_jid = os.getenv("MRCV_GROUP_JID", "120363430592783067@g.us")
                send_mrcv_wa_notif(msg, "MRCV_HEDGE_DETECT", target_jid=mrcv_group_jid, include_header=False)
                
            # --- CEK CLOSE ALL HANYA JIKA MRCV_WAIT_FOR_RCS_HEDGE=true & SEDANG HEDGE DI RCS ---
            if wait_for_hedge and is_rcs_hedge:
                ...
                # 1. KONDISI SUKSES: TARGET PROFIT TERCAPAI (>= $0.0)
                if total_net >= target_profit:
                    ...
                    mrcv_group_jid = os.getenv("MRCV_GROUP_JID", "120363430592783067@g.us")
                    send_mrcv_wa_notif(success_msg, "MRCV_SUCCESS", target_jid=mrcv_group_jid, include_header=False)
                ...
                # 2. KONDISI DARURAT: BATAS MAX LOSS TERCAPAI (EMERGENCY CUTLOSS REALTIME <= -$15.0)
            ...
            if mrcv_state.phase == MRCVPhase.IDLE:
                ...
                if all_current_positions:
                    if not wait_for_hedge:
                        ...
                        # Mode Recovery RCS: Posisi manual (magic bukan RCS dan bukan MRCV) memblokir recovery
            ...
            mrcv_group_jid = os.getenv("MRCV_GROUP_JID", "120363430592783067@g.us")
            send_mrcv_wa_notif(stop_msg, "MRCV_STOP", target_jid=mrcv_group_jid, include_header=False)

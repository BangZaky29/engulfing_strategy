...
def notify_engulfing_system_status(status: str, extra_info: str = ""):
    ...
    HANYA dikirim ke group SKIP_SIGNAL (GROUP-MALING-SKIPPED).
    ...
    try:
        ...
        # Row HANYA untuk Group SKIPPED Engulfing
        if skip_maling_jid:
            outbox_rows.append({
        ...
        if outbox_rows:
            ...
            print(cprint(f"📲 Notifikasi Engulfing System ({status}) terkirim ke {skip_maling_jid}", Colors.GREEN))
